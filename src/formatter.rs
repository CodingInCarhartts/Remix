...
fn format_markdown(repo: &PackedRepository) -> String {
    ...
    output.push_str(&format!("- **Binary Files:** {}\n", repo.summary.binary_file_count));
    ...
    if let Some(suspicious_files) = &repo.suspicious_files {
        if !suspicious_files.is_empty() {
            ...
            output.push_str(&format!("⚠️ **{} suspicious file(s) detected that may contain sensitive information:**\n\n", suspicious_files.len()));
            
            for (i, file) in suspicious_files.iter().enumerate() {
    ...
    if let Some(binary_files) = &repo.binary_files {
        if !binary_files.is_empty() {
            output.push_str("\n## Binary Files\n\n");
            output.push_str("The following binary files were detected but not included in the content:\n\n");
    ...
    let mut files_by_dir: std::collections::BTreeMap<String, Vec<&crate::packer::FileContent>> = std::collections::BTreeMap::new();
...
fn format_json(repo: &PackedRepository) -> Result<String> {
    serde_json::to_string_pretty(repo)
        .context("Failed to serialize repository to JSON")
}
...
fn format_text(repo: &PackedRepository) -> String {
    ...
    if let Some(suspicious_files) = &repo.suspicious_files {
        if !suspicious_files.is_empty() {
            output.push_str("\nSECURITY CHECK RESULTS:\n\n");
            output.push_str(&format!("WARNING: {} suspicious file(s) detected that may contain sensitive information:\n\n", suspicious_files.len()));
